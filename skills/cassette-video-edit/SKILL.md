...
- Showing previews in a terminal host (Claude Code, Codex CLI): the timeline digest is text — when the user asks to see the timeline, reprint `quality.timeline_ctl` verbatim in a fenced code block in your reply, never a paraphrase. For the contact sheet or storyboard sheet, terminal hosts cannot render image results inline — if `chafa` is available, render the sheet into the terminal with `chafa -f symbols --symbols all --size <cols>x<rows>` sized to the visible window (the user can expand collapsed output with ctrl+o), and always give the sheet's file path so the user can open the real pixels.
